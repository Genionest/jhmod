...
local function ShouldRunAway(inst, target)
    return not inst.components.trader:IsTryingToTradeWithMe(target) and not target:HasTag("pigroyalty")
end
...
function PigBrain:OnStart()
    ...
    local day = WhileNode( function() return clock and clock:IsDay() end, "IsDay",
        PriorityNode{
            ...
            WhileNode(function()
                return self.inst:HasTag("must_chop_tree")
            end, "must_chop", LoopNode{
