...
local TpLevelScreen = Class(Screen, function(self, data, owner)
	SetPause(true)
	self.data = data
    self.owner = owner
	Screen._ctor(self, "TpLevelScreen")
	...
	self.balance_bg:AddChild(Image(
		"images/inventoryimages/sam_items.xml", "tp_epic.tex"
	))
	...
	self.level_icon = self.level_bg:AddChild(ImageButton(
		"images/inventoryimages.xml", "accomplishment_shrine.tex"
	))
    self.level_icon:SetOnClick(function()
        self.data:Sure()
		TheFrontEnd:PopScreen(self)
		SetPause(false)
    end)
...
function TpLevelScreen:ItemBoardPageTurn(dt)
...
function TpLevelScreen:SetItemBoard()
...
function TpLevelScreen:SetBalance()
...
function TpLevelScreen:SetLevel()
	-- local level = self.data:GetLevel(self)
	-- self.level:SetString(level)
end

function TpLevelScreen:Test()

end

return TpLevelScreen