...
local w_FollowImg_g = w_require_g("extension/uis/follow_image")
local w_v02_g = .2
local w_print_g = print
local w_A_g = "a"
local w_v465_g = w_v100_g*(w_v2_g+w_v1_g+w_v0_g)+w_v100_g+w_v30_g*w_v2_g + w_v3_g*2-w_v1_g

local w_WgFollowImage_g = w_Class_g(function(w_self_g, w_inst_g)
	w_self_g["inst"] = w_inst_g
	-- :SetImage(atlas, img, fn)
	w_self_g["SetImage"] = function(w_self_g, w_atlas_g, w_img_g, w_fn_g)
		w_self_g["inst"]["DoTaskInTime"](w_self_g["inst"], 0, function()
			if w_self_g["widget"] == w_nil_g then
				...
				if w_player_g["HUD"] then
					w_self_g["widget"] = w_player_g["HUD"]["AddChild"](w_player_g["HUD"], w_FollowImg_g(w_atlas_g, w_img_g))
	...
	w_self_g["SetScale"] = function(w_self_g, ...)
		if w_self_g["widget"] then
			w_self_g["widget"]["wg_image"]["SetScale"](w_self_g["widget"]["wg_image"], ...)
		end
	end
	-- :Show()
