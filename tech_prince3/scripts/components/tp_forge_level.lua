...
local TpForgeLevel = Class(function(self, inst)
    self.inst = inst
    self.level = 1
...
function TpForgeLevel:SetAttrFactor(attr, factor)
...
function TpForgeLevel:LevelUp()
    self.level = self.level + 1
end

function TpForgeLevel:CanForge(material)
...
function TpForgeLevel:Forge(material)
    local n = 1
    if self.forge_material == "tp_infused_nugget_black" then
        n = self.level
    end
...
function TpForgeLevel:CanBeElemental()
...
function TpForgeLevel:SetElement(element)
    self.element = element
end

function TpForgeLevel:OnSave()
    return {
        level = self.level,
...
function TpForgeLevel:OnLoad(data)
    if data then
...
function TpForgeLevel:GetWargonString()
...
function TpForgeLevel:GetWargonStringColour()
