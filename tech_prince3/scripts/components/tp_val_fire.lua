...
local TpValFire = Class(WgValue, function(self, inst)
    WgValue._ctor(self, inst)
    self:SetRate(-1)
    self.event = "val_fire_delta"
    self.badge = nil
    self:Start()
end)
...
-- function TpValFire:GetWargonString()
