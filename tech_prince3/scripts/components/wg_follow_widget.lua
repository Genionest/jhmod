...
local w_WgFollowWidget_g = w_Class_g(function(w_self_g, w_inst_g)
	w_self_g["inst"] = w_inst_g
	-- :SetAnim(bank, build, anim, fn)
	w_self_g["SetWidget"] = function(w_self_g, w_widget_g, w_fn_g)
		w_self_g["inst"]["DoTaskInTime"](w_self_g["inst"], 0, function()
			if w_self_g["widget"] == w_nil_g then
				...
				if w_player_g["HUD"] then
					w_self_g["widget"] = w_player_g["HUD"]["AddChild"](w_player_g["HUD"], w_widget_g)
    ...
    w_self_g["GetWidget"] = function(w_self_g)
        return w_self_g["widget"]
    end
