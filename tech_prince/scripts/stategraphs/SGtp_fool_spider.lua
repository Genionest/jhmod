...
local states=
{
	...
	State{
        ...
        onenter = function(inst, cb)
            ...
            if target then
                -- for i = 1, inst.spawned or 1 do
                    local pos = WARGON.around_land(target, 1)
                    if pos then
                        WARGON.make_fx(pos, "tp_fx_splash_ground_pound")
                    end
                -- end
            end
        end,
 ...
 --            TimeEvent(64*FRAMES, function(inst)
	...
	-- 			if spider then
 --                    local offset = FindGroundOffset(pt, -angle, rad, 30) or Vector3(rad*math.cos(angle), 0, rad*math.sin(angle))
 --                    spider.Transform:SetPosition((pt + offset):Get())
...
return StateGraph("tp_fool_spider", states, events, "birth", actionhandlers)
