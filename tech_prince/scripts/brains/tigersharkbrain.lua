...
local function FindWaterAction(inst)
    ...
    local wateroffset = FindWaterOffset(inst:GetPosition(), math.random() * 2 * math.pi, 30*20, 36*20)
    if wateroffset then
    ...
    local root = PriorityNode(
    {
        ...
        WhileNode(function() return self.inst.components.health:GetPercent() <= 0.1 end, "Low Health",
            PriorityNode({
                ...
                Leash(self.inst, function() return self.inst:FindSharkHome() and self.inst:FindSharkHome():GetPosition() end, 810, 800, true),
            }, 0.25)),
