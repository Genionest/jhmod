...
local function fn(Sim)
	...
	minimap:SetIcon( "tp_cook_pot.tex" )
	...
	inst.AnimState:SetBank("cook_pot_warly")
	-- inst.AnimState:SetBuild("cook_pot_warly")
	inst.AnimState:SetBuild("tp_cook_pot")
	inst.AnimState:PlayAnimation("idle_empty")
	...
	local function cookpot_use(inst)
		WARGON.make_fx(inst, "collapse_small")
		inst.components.tpbebird:BeBird()
	end

	local function cookpot_test(inst, doer)
		return not (inst.components.stewer.done or inst.components.stewer.cooking)
	end

	WARGON.CMP.add_cmps(inst, {
		-- use = {use=cookpot_use, test=cookpot_use_test},
		-- machine = {on=cookpot_use, test=cookpot_test},
		tpbebird = {},
		})
...
return Prefab( "common/tp_cook_pot", fn, assets, prefabs)
